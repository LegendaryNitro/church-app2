...
function Chat() {
  ...
  // Add state variables for room management
  ...
  useEffect(() => {
    ...
    // Listen for room creation and user joining events
  ...
  const recordNow = () => {
    if (stream && !recording) {
      ...
      recorder.onstop = () => {
        ...
        // Store the recorded video URL in state
  ...
  const handleCopyToClipboard = () => {
    alert(`ID of ${me} copied!`);
  };

  // Function to create a room
  const createRoom = () => {
    socket.emit("create-room", { hostId: me });
  };

  // Function to join a room using its room ID
  ...
  return (
    <div>
      <h1 style={{ textAlign: "center", color: '#fff' }}>Malonlin Video conferencing</h1>
          ...
          {noCall && callAccepted ? (
            <div>
              {recording ? (
                ...
                <Button variant="contained" color="primary" onClick={recordNow}>
                  Record
                </Button>
              )}
            </div>
