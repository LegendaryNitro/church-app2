...
router.post("/", async (req, res) => {
	try {
		...
		let user = await UserMVELI.findOne({ email: req.body.email });
		if (!user)
			return res
				.status(409)
...
router.get("/:id/:token", async (req, res) => {
	try {
		const user = await UserMVELI.findOne({ _id: req.params.id });
...
router.post("/:id/:token", async (req, res) => {
	try {
		...
		// const salt = await bcrypt.genSalt(Number(process.env.PASS_SEC));
		const hashPassword = await CryptoJS.AES.encrypt(req.body.password, process.env.PASS_SEC).toString();
